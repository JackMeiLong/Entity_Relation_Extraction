...
from model_v2 import CustomModel
import numpy as np
import yaml
import os
...
def create_dataset(data_path, batch_size, data_type):
    ...
    if data_type != 'concat':
        ...
        obj_type = np.asarray(dataset[:, 6].tolist()).astype('int')
        sub_type = np.asarray(dataset[:, 7].tolist()).astype('int')
        custom_dataset = TensorDataset(torch.tensor(input_ids), torch.tensor(attention_mask), torch.tensor(token_type_ids), 
        torch.tensor(obj_type_idx), torch.tensor(sub_type_idx), 
        torch.LongTensor(obj_type), torch.LongTensor(sub_type),
    ...
    for epoch in range(train_config['max_epochs']):
        ...
        for step, batch in enumerate(train_dataset, 1):
            ...
            if data_type != 'concat':
                ...
                obj_type = batch[5].cuda().long() if torch.cuda.is_available() else batch[5].long()
                sub_type = batch[6].cuda().long() if torch.cuda.is_available() else batch[6].long()

            _, loss, metric = model(input_ids, attention_mask, token_type_ids, obj_type_idx, sub_type_idx, obj_type, sub_type, output_y)
...
if __name__ == '__main__':
    ...
    conf_file = open('./configs_v2.yml', 'r', encoding='utf-8')
    conf = conf_file.read()
    conf_file.close()
    ...
    train_dataset = create_dataset('../data/data_source/train_data_v2.npy', train_config['batch_size'], data_type)
    dev_dataset = create_dataset('../data/data_source/dev_data_v2.npy', train_config['batch_size'], data_type)

    model = CustomModel(model_config['bert_path'][bert_type], model_config['hidden_size'], train_config['fine_tune'], model_config, type_size,49)

    if data_type != 'concat':
        conf_file = open('../data/data_config_v2.yml', 'r', encoding='utf-8')
        conf = conf_file.read()
        conf_file.close()
